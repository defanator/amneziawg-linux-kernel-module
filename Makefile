...
GITHUB_REPOSITORY ?= amnezia-vpn/amneziawg-linux-kernel-module
GITHUB_RUN_ID     ?= 0
...
update-version: ## Update version in sources
	...
	sed -i -e 's,#define WIREGUARD_VERSION ".*,#define WIREGUARD_VERSION "$(VERSION)",' src/version.h ; \
	sed -i -e 's,^WIREGUARD_VERSION =.*,WIREGUARD_VERSION = $(VERSION),' src/Makefile ; \
	sed -i -e 's,^PACKAGE_VERSION=".*,PACKAGE_VERSION="$(VERSION)",' src/dkms.conf ; \
	sed -i -e 's,^Version:.*,Version: $(VERSION),' amneziawg-dkms.spec ; \
	}
...
update-version-verbose: ## Update version with full build references
	...
	sed -i -e 's,#define WIREGUARD_VERSION ".*,#define WIREGUARD_VERSION "$(_LONG_VERSION)",' src/version.h ; \
	sed -i -e 's,^WIREGUARD_VERSION =.*,WIREGUARD_VERSION = $(_LONG_VERSION),' src/Makefile ; \
	sed -i -e 's,^PACKAGE_VERSION=".*,PACKAGE_VERSION="$(VERSION)",' src/dkms.conf ; \
	}

build: update-version-verbose ## Build kernel module
	$(MAKE) -C src
...
dist/rpmbuild: update-version
	...
	mkdir -p $@/SPECS
	mkdir -p $@/SOURCES
	tar -czvf $@/SOURCES/v$(VERSION).tar.gz --transform 's|^|amneziawg-linux-kernel-module-$(VERSION)/|' src/ README.md COPYING
	cp amneziawg-dkms.spec $@/SPECS/
	sed -i -e "s,%changelog,%changelog\n* $$(date +"%a %b %d %Y") $(WHOAMI) <$(WHOAMI)@$(HOSTNAME)> - $(VERSION)\n- automated build ($(LONG_VERSION))\n- $(_RUN_URL)\n," $@/SPECS/amneziawg-dkms.spec

build-rpm: update-version dist/rpmbuild ## Build .rpm package
	rpmbuild -D "_topdir $(TOPDIR)/dist/rpmbuild" -ba dist/rpmbuild/SPECS/amneziawg-dkms.spec

builder-%: ## Create containerized builder
	docker build \
		-f $(TOPDIR)/containers/Containerfile.$* \
		-t amneziawg-linux-kernel-module-builder:$* \
		$(TOPDIR)/containers

buildenv-%: ## Run building environment in a container
	docker run --rm \
		-v $(TOPDIR):/amneziawg-linux-kernel-module \
		-ti amneziawg-linux-kernel-module-builder:$* \
		bash

.PHONY: clean
clean: ## Clean up
	rm -rf $(TOPDIR)/dist
	git checkout src/version.h src/Makefile src/dkms.conf amneziawg-dkms.spec
	$(MAKE) -C src clean
